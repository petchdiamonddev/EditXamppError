...
def copy_backup(xampp_path):
    backup_dir = os.path.join(xampp_path, 'mysql', 'backup')
    data_dir = get_data(xampp_path)

    if os.path.exists(backup_dir):
        # ลบโฟลเดอร์ data เดิมก่อนคัดลอกเพื่อป้องกันข้อผิดพลาด
        ...
        if item not in exclude_dirs:
            if os.path.exists(dst_path):
                ...
                if os.path.isdir(src_path):
                    ...
                    print(f'คัดลอกโฟลเดอร์ {src_path} ไปยัง {dst_path}')
                else:
                    shutil.copy2(src_path, dst_path)
                    print(f'คัดลอกไฟล์ {src_path} ไปยัง {dst_path}')
...
def main():
    ...
    new_data_dir = rename_data(data_dir)

    # ขั้นตอนที่ 2: คัดลอก backup ไปยัง data
    copy_backup(xampp_path)
